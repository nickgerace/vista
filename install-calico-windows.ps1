...
Param(
    [parameter(Mandatory = $false)] $ReleaseBaseURL="https://github.com/projectcalico/calico/releases/download/v3.19.0/",
    [parameter(Mandatory = $false)] $ReleaseFile="calico-windows-v3.19.0.zip",
    [parameter(Mandatory = $false)] $KubeVersion="1.20.6",
    [parameter(Mandatory = $false)] $DownloadOnly="yes",
    ...
    [parameter(Mandatory = $false)] $ServiceCidr="10.43.0.0/16",
    [parameter(Mandatory = $false)] $DNSServerIPs="10.43.0.10",
    [parameter(Mandatory = $false)] $CalicoBackend="backend"
)
...
function DownloadFiles()
{
    ...
    Invoke-WebRequest https://github.com/rosskirkpat/win-binaries/raw/main/host-local.exe -OutFile $BaseDir\cni\host-local.exe 
    New-Item -ItemType Directory -Path c:\opt\ -Force > $null
    New-Item -ItemType Directory -Path c:\opt\cni -Force > $null
    New-Item -ItemType Directory -Path c:\opt\cni\bin -Force > $null
    New-Item -ItemType Directory -Path c:\opt\cni\conf -Force > $null
    copy-item $BaseDir\cni\host-local.exe C:\opt\cni\bin\
    ...
    # RK - need to review and possibly update these scripts and dockerfile
...
if ($platform -EQ "eks") {
    ...
    $calicoNs = GetCalicoNamespace -KubeConfigPath C:\k\kubeconfig
    GetCalicoKubeConfig -CalicoNamespace $calicoNs -KubeConfigPath C:\k\kubeconfig
}
if ($platform -EQ "ec2") {
    $awsNodeName = Invoke-RestMethod -uri http://169.254.169.254/latest/meta-data/local-hostname -ErrorAction Ignore
